...
class Menu extends Model
{
    ...
    public function getJsTree($role_id = null){
		$model =new self;
		$list =$model->order('sort asc,id asc')->where(['ishow'=>1,'isopen'=>1,'isdel'=>1])->column("*",'id');
        // dump($list);die;
    ...
    // 获取菜单列表
    public function getmenulist($pid=0,$fields=''){
        // $model = db('menu');
        $model = new self;
        $where = [
            'pid'=>$pid,
        ...
        $lists = $model->where($where)->order('sort asc,id asc')->field($fields)->select();

        foreach ($lists as $k => $val) {
            $lists[$k]['son'] = $this->getmenulist($val['id'], $fields);
        }
