...
class Comment extends BaseModel
{
    ...
    {
        return db('comment')->where(array('id'=>$comment_id))->find();
    }
    //    获取该评价的所有回复
    public static function reply($comment_id)
    {
        return db('comment')->where(array('parent_id'=>$comment_id))->select();
    }
    //    删除用户评价以及管理员回复
    public static function delusercomment($comment_id)
    {
        $data = self::get_comment($comment_id);
        if($data){
           $res1 =  db('comment')->where('id',$comment_id)->update(['is_delete'=>1]);
           if($res1){
               return true;
           }else{
               return false;
