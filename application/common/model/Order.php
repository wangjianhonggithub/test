...
class Order extends BaseModel
{
    ...
    // 获取临时未付款订单
    public function get_temporder($uid,$sid){
        return db('order_temp')->where(['uid'=>$uid,'sid'=>$sid, 'stype'=>1])->find();
    }
    // 新增临时订单
    public function add_temporder($uid, $data, $price){
        $paytype = 1;
        if($data['cpsid'] > 0){
            $paytype = 2;   // 优惠券满减
        }
        ...
        $add = db('order_temp')->insert($add_data);
        if($add){
            return ajax(200, $add_data['oid']);
        }else{
            return ajax(400);
        }
    }
    ...
    // 编辑临时订单
    public function mod_temporder($uid, $data, $price){
        $paytype = 1;
        if($data['cpsid'] > 0){
            $paytype = 2;   // 优惠券满减
        }
        ...
        $mod = db('order_temp')->where(['uid'=>$uid,'sid'=> $sid])->update($mod_data);
        if($mod){
            return ajax(200, $mod_data['oid']);
        }else{
            return ajax(400);
        }
    }
    ...
    // 获取订单列表
    public function getorder_list($page, $url, $uid='',$kwords = ''){
        $model = db('order');
        $where[] = ['isdel', '=', 1];
        ...
        if (!empty($kwords)) {
            if (is_array($kwords)) {
                ...
                // 支付方式
                if ($kwords['paytype']) {
                    $where[] = ['paytype', '=', $kwords['paytype']];
                }
                // 课程名称
                if ($kwords['course_name']) {
                    $where[] = ['course_name', 'like', '%'.$kwords['course_name'] . '%'];
                }
                // 参课人
                if ($kwords['number']) {
                    $where[] = ['number', '=', $kwords['number']];
                }
                // 状态
                if ($kwords['stype']) {
        ...
        if ($page == 0) {
            $fields =  $url == '' ? 'id,oid,uid,sid,phone,number,course_name,course_date,course_stime,course_etime,price,addstamp,paytype,stype' : $url;
            // 获取全部
            $lists = $model->where($where)->order($sort)->field($fields)->select();
            foreach ($lists as $k => $val) {
                $lists[$k]['course_date'] = date("Y-m-d", $val['course_date']);
                $lists[$k]['course_week'] = '星期' . $weekarray[date("w", $val['course_date'])];
                $lists[$k]['course_stime'] = substr($val['course_stime'], 0, 5);
                $lists[$k]['course_etime'] = substr($val['course_etime'], 0, 5);
                $lists[$k]['addstamp'] = date("Y-m-d H:i", $val['addstamp']);
            }
            return $lists;
        }
        ...
        foreach ($lists as $k => $val) {
            ...
            $lists[$k]['course_stime'] = substr($val['course_stime'], 0, 5);
            $lists[$k]['course_etime'] = substr($val['course_etime'], 0, 5);
            $lists[$k]['userinfo'] = db('user')->where(['uid'=>$val['uid']])->field('nickname,headimg')->find();
            $lists[$k]['classname'] = db('courseclass')->where(['id'=>$val['course_cid']])->value('name');
    ...
    // 订单过期
    public function order_past($uid = ''){
        if ($uid) {
            $pastwhere[] = ['uid', '=', $uid];
        }
        $pastwhere[] = ['pastamp', '<=', time()];
        db('order')->where($pastwhere)->update(['stype' => 2, 'modstamp' => time()]);
    }

    public function getorder_one($oid,$fields){
        if ($oid) {
            return db('order')->where(['oid' => $oid])->field($fields)->find();
        }
    }
...
//////////
    ...
    */
        ...
        foreach ($list as $order) {
            ...
            foreach ($order['goods'] as $Item) {
                $dataArray[] = [
                    ...
                    '物流公司' => $this->filterValue(isset($order['express']['express_name'])?$order['express']['express_name']:'-'),
                    '物流单号' => $this->filterValue(isset($order['express']['express_no'])?$order['express']['express_no']:'-'),
                    ...
                    '发货时间' => $this->filterTime(isset($order['delivery_time'])?$order['delivery_time']:'0'),
                    '收货状态' => $this->filterValue($order['receipt_status']['text']),
                    '收货时间' => $this->filterTime(isset($order['receipt_time'])?$order['receipt_time']:'0'),
    ...
    {
        // 导出csv文件
        $filename = 'delivery-' . date('YmdHis',time());
