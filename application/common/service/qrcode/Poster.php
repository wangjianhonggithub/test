...
{
    /* @var \app\common\model\dealer\User $dealer */
    private $dealer;
    ...
    public function __construct($dealer)
    {
        ...
        $this->dealer = $dealer;
        // 分销商海报设置
        $this->config = Setting::getItem('qrcode', $dealer['app_id']);
    ...
    {	
        ...
        // 2. 下载用户头像
        $avatarUrl = $this->saveTempImage($appId, $this->dealer['avatarUrl'], 'avatar');
		
        // 3. 下载小程序码
        $qrcode = $this->saveQrcode($appId, 'uid:' . $this->dealer['user_id']);
    ...
    private function getPosterPath()
    {
        ...
        // 保存路径
        $tempPath = WEB_PATH . 'uploads' . '/' . $this->dealer['app_id'] . '/';
    ...
    {
        return $this->posterName ?: md5('poster_' . $this->dealer['user_id']) . '.png';
    }
    ...
    private function getPosterUrl()
    {
        ...
        return $this->posterUrl ?: (\base_url() . 'uploads/' . $this->dealer['app_id'] . '/' . $this->getPosterName());
    }
