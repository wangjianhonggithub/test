...
class Item extends Controller
{
	...
	**/
    ...
    {
        $comment_id = input('comment_id/d', 0, 'intval');
        $model = CommentModel::get_comment($comment_id);
        if (!$this->request->isAjax()) {
            $reply = CommentModel::reply($comment_id); // 评论回复列表

            $this->assign('comment',$model);
            $this->assign('reply',$reply);
            return $this->fetch('reply');
        }
        $session = session('dzcadmin');
        $comment = $_POST['comment_id'];
        $data = CommentModel::get_comment($comment);
        $add['parent_id'] = $data['id'];
        $add['content'] = trim($_POST['content']);
        $add['item_id'] = $data['item_id'];
        $add['create_time'] = time();
        $add['update_time'] = time();
        $add['user_name'] = $session['user_name'];
        $add['user_id'] = $session['user_id'];
        $add['status'] = 1;
        $row =  db('comment')->insert($add);
        if ($row){
            return $this->renderSuccess('回复成功', url('item/comment'));
        }else{
    ...
    //  删除用户评论
    public function delusercomment()
    {
        $comment_id = $_POST['id'];
        return $this->isdel_change($comment_id, 'comment');
