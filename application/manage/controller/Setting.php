...
class Setting extends Controller
{
    ...
    public function agent(){
		...
		$model = new SettingModel;
        if ($model->editAgent($this->postData('setting'))) {
            $status = $this->postData('setting')['basic']['is_open'] == 1 ? 1 : 2;
            db('menu')->where('name','分销')->update(['ishow'=>$status]);
    ...
    public function wxset(){
        $comm = AppModel::detail();
        if ($this->request->isAjax()) {
            $mod_data = [
                'appkey' => input('appkey/s','','trim'),
                'app_secret' => input('app_secret/s','','trim'),
                'mchid' => input('mchid/s','','trim'),
                'apikey' => input('apikey/s','','trim'),
            ];
            $mod = db('app')->where(['app_id'=>$this->dzcadmin['app_id']])->update($mod_data);
            if($mod) return $this->renderSuccess('更新成功');
            return $this->renderError('更新失败');
        }
        return $this->fetch('', compact('comm'));
    }
    ...
    public function helpcenter()
    {
        ...
        $data = SettingModel::get_helpcenter($page, url('helpcenter'));
    ...
    public function runanswer()
    {
        ...
        if (!$this->request->isAjax()) {
            ...
            if ($id) {
                $comm = SettingModel::get_one_date($id);
                $this->assign('comm', $comm);
            }

            return $this->fetch('setting/runanswer');
        }


        if ($id ? SettingModel::modanswer($id) : SettingModel::addanswer()) {
            return $this->renderSuccess('更新成功', url('helpcenter'));
        }
    ...
    public function answershow()
    {
        ...
        $res =  SettingModel::answershow($id);
        if($res){
            return $this->renderSuccess('操作成功');
        }else{
            return $this->renderError('操作失败');
