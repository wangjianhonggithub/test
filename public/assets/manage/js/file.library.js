;(function ($, window, document, undefined) {
    ...
    FileLibrary.prototype = {
        ...
        showLibraryModal: function () {
            var _this = this;
            _this.getJsonData({group_id: 0}, function (data) {
        ...
        moveFilesEvent: function () {
            ...
            // 绑定文件选中事件
            $groupSelect.on('click', '.group-select', function () {
                var groupId =$(".form-control").val()
                ...
                layer.confirm('确定移动选中的文件吗？', {title: '友情提示'}, function (index) {
                    var load = layer.load();
                    $.post(STORE_URL +'/library/moveFiles', {
                        group_id: groupId
                        , fileIds: fileIds
                    }, function (result) {
        ...
        deleteFilesEvent: function () {
            ...
            _this.$element.on('click', '.file-delete', function () {
                ...
                layer.confirm('确定删除选中的文件吗？', {title: '友情提示'}, function (index) {
                    var load = layer.load();
                    $.post(STORE_URL + '/library/deleteFiles', {
                        fileIds: fileIds
                    }, function (result) {
                        layer.close(load);
                        if (result.code === 1) {
        ...
        addGroupEvent: function () {
            ...
            _this.$element.on('click', '.group-add', function () {
                layer.prompt({title: '请输入新分组名称'}, function (value, index) {
                    var load = layer.load();
                    $.post(STORE_URL + '/library/addGroup', {
                        group_name: value,
        ...
        editGroupEvent: function () {
            ...
            _this.$element.find('.file-group').on('click', '.group-edit', function () {
                ...
                layer.prompt({title: '修改分组名称', value: $li.attr('title')}, function (value, index) {
                    var load = layer.load();
                    $.post(STORE_URL + '/library/editGroup', {
                        group_id: group_id
                        , group_name: value
                    }, function (result) {
                        layer.msg(result.msg);
        ...
        deleteGroupEvent: function () {
            ...
            _this.$element.find('.file-group').on('click', '.group-delete', function () {
                ...
                layer.confirm('确定删除该分组吗？', {title: '友情提示'}, function (index) {
                    var load = layer.load();
                    $.post(STORE_URL + '/library/deleteGroup', {
                        group_id: group_id
                    }, function (result) {
                        layer.msg(result.msg);
                        if (result.code === 1) {
        ...
        getJsonData: function (params, success) {
            ...
            // 获取文件库列表
            $.getJSON(STORE_URL +'/library/fileList', params, function (result) {
                layer.close(loadIndex);
                if (result.code === 1) {
