<?php /*a:2:{s:82:"E:\PHPTutorial\WWW\dzcyiranjianshen\application\manage\view\ordercourse\index.html";i:1572253661;s:71:"E:\PHPTutorial\WWW\dzcyiranjianshen\application\manage\view\layout.html";i:1572926623;}*/ ?>
<!DOCTYPE html>
<html lang="en">
            ...
            <form class="layui-form layui-col-md11" method="post" action="">
					...
					<div class="layui-inline layui-col-md3">
						...
						<div class="layui-input-inline">
							<select class="layui-inline " lay-search data-layer-selected="{btnSize: 'sm', placeholder: '支付方式'}" name="paytype" >
								<option value="0" <?php if(($para["paytype"]==0)): ?>selected<?php endif; ?>>全部 </option> 
								<option value="1" <?php if(($para["paytype"]==1)): ?>selected<?php endif; ?>>全款支付</option> 
								<option value="2" <?php if(($para["paytype"]==2)): ?>selected<?php endif; ?>>优惠券满减</option>
								<option value="3" <?php if(($para["paytype"]==3)): ?>selected<?php endif; ?>>课程包抵扣</option>
							</select> 
						</div> 
					</div>
                </div>
                ...
                <div class="layui-form-item">
                    <div class="layui-inline layui-col-md4">
                        ...
                        <div class="layui-input-inline" >
                            <input name="course_name" type="text" class="layui-input" placeholder="请输入课程名称" value="<?php echo htmlentities($para['course_name']); ?>">
                        </div>
                    </div>
                    ...
                    <div class="layui-inline layui-col-md4">
                        ...
                        <div class="layui-input-inline" >
                            <select class="layui-inline " lay-search data-layer-selected="{btnSize: 'sm', placeholder: '人数'}" name="number" >
								<option value="0" <?php if(($para["number"]==0)): ?>selected<?php endif; ?>>全部 </option> 
								<option value="1" <?php if(($para["number"]==1)): ?>selected<?php endif; ?>>1人</option> 
								<option value="2" <?php if(($para["number"]==2)): ?>selected<?php endif; ?>>2人</option>
							</select> 
                        </div>
                    </div>
					...
					<div class="layui-inline layui-col-md3">
						...
						<div class="layui-input-inline">
							<select class="layui-inline " lay-search data-layer-selected="{btnSize: 'sm', placeholder: '状态'}" name="stype" >
								...
								<option value="1" <?php if(($para["stype"]==1)): ?>selected<?php endif; ?>>预约中</option> 
								<option value="2" <?php if(($para["stype"]==2)): ?>selected<?php endif; ?>>已结束</option>
							</select> 
						</div> 
					</div>
                </div>
                ...
                <table class="layui-table">
                    ...
                    <tbody>
                        ...
                        <?php else: foreach($data['list'] as $value): ?>
                                <tr>
                                    <td><?php echo htmlentities($value['oid']); ?></td>
									<td>
                                        <a href="<?php echo htmlentities($value['userinfo']['headimg']); ?>" title="点击查看大图" target="_blank"><img src="<?php echo htmlentities($value['userinfo']['headimg']); ?>" width="60" height="60" alt=""></a>
                                        <?php echo htmlentities($value['userinfo']['nickname']); ?>
                                    </td>
                                    <td><?php echo htmlentities($value['phone']); ?></td>
                                    <td><?php echo htmlentities($value['number']); ?></td>
                                    <td>
                                        <?php echo htmlentities($value['classname']); ?>
                                        <br><?php echo htmlentities($value['course_name']); ?>
                                        <br><?php echo htmlentities($value['course_date']); ?> <?php echo htmlentities($value['course_stime']); ?>-<?php echo htmlentities($value['course_etime']); ?>
                                    </td>
                                    <td>
                                        订单金额：￥<?php echo htmlentities($value['course_price']*$value['number']); ?>
                                        <hr>
                                        支付方式：
                                        <?php
                                        ...
                                        ?>
                                        <hr>
                                        实际付款：￥<?php echo htmlentities($value['price']); ?>
                                    </td>
                                    <td><?php echo date("Y-m-d H:i",$value['addstamp']); ?></td>
                                    <td><?php echo $value['stype']==1 ? '预约中' : '已结束'; ?></td>
                                    <td>
                                        <div class="tpl-table-black-operation">
											<a href="javascript:;" onclick="orderdetail('<?php echo htmlentities($value['oid']); ?>')">详情</a>
                                            <a href="javascript:;" class="value-delete tpl-table-black-operation-del delcomm " data-id="<?php echo htmlentities($value['id']); ?>">删除</a>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
...
<script>
    $(function () {
        $('.delcomm').delete('id', "<?php echo url('orderdel'); ?>", '数据删除后无法恢复,确认删除？');
    });
    ...
    function orderdetail(oid){
        layer.open({
            ...
            area: ['1200px', '80%'],
            content: "<?php echo url('detail','',''); ?>&oid=" + oid, //iframe的url    
            end: function () {
                // location.reload();
            }
        });
